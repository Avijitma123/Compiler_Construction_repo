...
void printMaximalTokens(char *input) {
    ...
    TokenInfo tokens[n + 1]; // Array to store tokens
    int numTokens = 0;
        ...
        for (int j = 0; j < i; j++) {
            if (isToken(input, j, i - 1)) {
                ...
                if (dp[i] < dp[j] + tokenLength) {
                    ...
                    // Store the token
                    tokens[numTokens].string = input + j;
                    tokens[numTokens].length = tokenLength;
                    numTokens++;
                }
            }
        }
    ...
    // Print the sequence of maximal length tokens in the correct order
    for (int i = numTokens - 1; i >= 0; i--) {
        printf("<%.*s>", tokens[i].length, tokens[i].string);
        if (i > 0) {
            printf(" ");
        }
    }
